...
for (nLesson in 1:n_lesson){
  ...
  for (i in 1:nrow(names_user)){
    ...
    df_IEA514_lesson[i] <- as.character(total_score$lesson[nLesson])
  ...
  df_IEA514_results_all <- cbind(df_IEA514_results_all,df_IEA514_results)
}



write.table(df_IEA514_results_all,'IEA514_temp.csv',sep=',')
